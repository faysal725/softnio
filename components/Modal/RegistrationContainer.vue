    ...
    <Dialog class="relative z-50" >
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
        ...
        >
          ...
          >
            <DialogPanel
              class="relative transform overflow-visible rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:p-6 h-full md:h-fit w-full md:w-fit"
              :class="classes"
            >
              <div  class=" flex  justify-end"> 
                <XMarkIcon @click="close()" class="h-5 w-5 text-slate-500 dark:text-slate-200 cursor-pointer" />
              </div>
              <slot />
            </DialogPanel>
          </TransitionChild>
